...
export function LiveExtractionView({
  ...
  return (
    <div className="mt-6 bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
      ...
      {/* Reconnecting — connection dropped (backgrounded tab / flaky network).
          We're recovering automatically; this is not an error state. */}
      {state.status === 'reconnecting' && (
        <div className="flex items-center gap-2 px-5 py-2.5 text-xs border-b border-[var(--border)] bg-amber-50 text-amber-800">
          <Wifi className="w-3.5 h-3.5 animate-pulse" />
          Reconnecting — your progress is safe. Picking up where it left off…
        </div>
      )}
      ...
      {state.status === 'complete' && state.result && (
        <div className="p-5 border-t border-[var(--border)] space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            {shareUrlFor(state) && (
              <Link
                href={shareUrlFor(state)!}
                className="fm-btn inline-flex items-center justify-center gap-1.5 bg-[var(--foreground)] text-[var(--background)] font-semibold px-4 py-2.5 rounded-xl hover:bg-[var(--accent)] text-sm"
              >
                Open results page
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 pt-1">
            <Link
              href="/atlas"
              className="text-xs font-semibold text-[var(--muted)] hover:text-[var(--accent)] inline-flex items-center gap-1"
            >
              View on atlas
...
/** Public results page for the completed run, or null if no videoId yet. */
function shareUrlFor(state: StreamState): string | null {
  const videoId = state.result?.videoId ?? state.video?.videoId
  return videoId ? videoSharePath(videoId) : null
}
